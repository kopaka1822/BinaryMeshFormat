...
TEST(TestSuite, InstanceMerge)
{
	...
	glm::mat4 transMat = glm::rotate(glm::translate(glm::mat4(1.0f), glm::vec3(1.0f, 2.0f, 3.0f)), 3.141f, glm::vec3(1.0f, 0.0f, 0.0f));
	const auto stride = getAttributeElementStride(Position | Texcoord0);
	for(size_t i = 0; i < vertices2.size(); i += stride)
	{
		auto res = transMat * glm::vec4(vertices2[i], vertices2[i + 1], vertices2[i + 2], 1.0f);
		vertices2[i] = res.x;
		vertices2[i + 1] = res.y;
		vertices2[i + 2] = res.z;
		EXPECT_TRUE(std::abs(1.0f - res.w) < 0.00001f);
	}
	...
	auto transMat2 = meshes[0].getInstanceTransforms()[1];
	// transform matrices
	
	for (glm::length_t i = 0; i < 4; ++i)
		for (glm::length_t j = 0; j < 4; ++j)
			EXPECT_LT(std::abs(transMat[i][j] - transMat2[i][j]), 0.0001f);
