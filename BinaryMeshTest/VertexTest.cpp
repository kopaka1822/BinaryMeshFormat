...
TEST(TestSuite, Get)
{
	...
	ASSERT_EQ(v.get(Position)[0], data1[0]);
	ASSERT_EQ(v.get(Position)[1], data1[1]);
	ASSERT_EQ(v.get(Position)[2], data1[2]);

	ASSERT_EQ(v.get(Texcoord0)[0], data1[3]);
	ASSERT_EQ(v.get(Texcoord0)[1], data1[4]);

	ASSERT_EQ(v.get(Texcoord2)[0], data1[5]);
	ASSERT_EQ(v.get(Texcoord2)[1], data1[6]);
}